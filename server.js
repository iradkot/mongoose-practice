...
// var max = 500;
// var min = 200;
// Book.find({ pages: { $gt: min , $lt: max}} , function (err, rslt) {
//   if (err) { return console.error(err); }
//   console.log(rslt);
// });
//2. Find books whose rating is less than 5, and sort by the author's name

// Book.find({ rating: {$lt: 5}}).sort('author').exec(function (err, rslt) {
//   if (err) { return console.error(err); }
//   console.log(rslt);
// });
//3. Find all the Fiction books, skip the first 2, and display only 3 of them 
// Book.find({ genres: "Fiction"}).sort('author').skip(2).limit(3).exec(function (err, rslt) {
//   if (err) { return console.error(err); }
//   console.log(rslt);
// });
...
// Person.find({ height: { $gt: 180 }, salary: { $gt: 30000 } }, function (err, rslt) {
//   if (err) { return console.error(err); }
//   console.log(rslt);
// });
//2. Find all the people who are tall (>180) OR rich (>30000)
// Person.find({ $or: [{ height: { $gt: 200 } }, { salary: { $gt: 40000 } }]}, function(err, rslt) {
//     if (err) { return console.error(err); }
//     console.log(rslt);
//   });
//3. Find all the people who have grey hair or eyes, and are skinny (<70)
// Person.find({ $or: [{ hair: "grey" }, { eyes: "grey" }], weight: { $lt: 70 } }, function (err, rslt) {
//   if (err) { return console.error(err); }
//   console.log(rslt);
// });
...
Person.find({ weight: { $gt: 100 }, kids: { $elemMatch: { weight: { $gt: 100 } } } }, function (err, rslt) {
  if (err) { return console.error(err); }
  console.log(rslt);
});
